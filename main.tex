...
then the resulting state is also a classical configuration, with spins on $\star{j}$ flipped.
...
to traverse the degenerate states starting from any one of them.
...
\]
allow the traversal, where the loop $l^*$ connects plaquette centers penetrating the edges.
...
Thus, $A_l$ negates the eigenvalues of the star operators at its endpoints,
raising the energy by $2J_e$.
One says that a site with $A_s = -1$ has an \emph{electric charge} at that site,
and thus interpret $J_e$ as the excitation energy of a single electric charge.
...
creating two vortices or \emph{magnetic charges}, each with excitation energy $J_m$.
