...
\renewcommand{\star}[1]{\mathrm{star}\left(#1\right)}
...
Again, it should be stressed that these boundary modes are exact zero modes only in the semi-infinite chain limit;
for finite values of $N$, the above sequences $\left\{u_j\right\}$ are not exact eigenvectors of $A$.
The logic is that if low-energy modes exist, then those eigenstates should be approximated by these boundary modes for sufficiently large $N$.
...
\[
    \mp E(k) := \mp \sqrt{
        \left(w\cos ka + \frac{\mu}{2}\right)^2
        + \left(\Delta\sin ka\right)^2},
\]
...
In fact, explicit calculation yields $\nu = \mathrm{sign}\left(\left(\frac{\mu}{2}\right)^2 - w^2\right)$,
...
\[
	\rho_X: G \rightarrow \mathrm{Homeo}(X)\quad \text{and}\quad \rho_Y: G \rightarrow \mathrm{Homeo}(Y).
\]
...
(Note that despite the notation, the equivariant homotopy depends on the chosen group actions on both $X$ and $Y$,
...
% =========================================
\section{The two-dimensional toric code}
The \emph{toric code} system is defined on a cellulation of a surface $\Sigma$.
We place a spin-$\frac{1}{2}$ on each edge and impose the Hamiltonian
\[
	H_T := -J_e \sum_s A_s - J_m \sum_p B_p,
\]
where $s$ runs over each vertex and $p$ over each face or \emph{plaquette}.
The star operators $A_s$ and the plaquette operators $B_p$ are defined as
\[
	A_s := \prod_{j \in \star{s}} \sigma_j^x,\quad
	B_p := \prod_{j \in \bdry{p}} \sigma_j^z.
\]
These operators all commute with each other.
In particular, a star and a plaquette share an even number of edges,
so the minus signs arising from the anticommutation of $\sigma^x$ and $\sigma^z$ cancel pairwise.

Since $A_s^2 = B_p^2 = I$, the simultaneous eigenspaces of all star and plaquette operators
are labeled by their eigenvalues $A_s, B_p \in \{\pm 1\}$.
This labeling is not independent, however, due to the constraints
\[
	\prod_s A_s = \prod_p B_p = I,
\]
as each edge has exactly two vertices and lies on the boundary of exactly two plaquettes.
Thus, for a connected closed surface, there are $V-1$ independent star operators and $F-1$ independent plaquette operators.

If the graph $G$ has $V$ vertices, $E$ edges, and $F$ plaquettes, the full Hilbert space is $2^E$-dimensional,
while there are $2^{V-1}\cdot 2^{F-1}$ distinct simultaneous eigenspaces.
Hence each such eigenspace has dimension
\[
	\frac{2^E}{2^{V+F-2}} = 2^{E-V-F+2} = 2^{2-\chi},
\]
where $\chi := V-E+F$ is the Euler characteristic of $\Sigma$.
For a genus-$g$ surface, $\chi=2-2g$, so in particular the ground-state sector,
corresponding to $A_s=B_p=+1$ for all $s, p$, is $4^g$-fold degenerate.\footnote{%
While the paper only states this result for genus-$g$ surfaces,
the definition of genuses depends on the surface being orientable.
If the surface is non-orientable, like the Klein bottle for example,
then the genus is ill-defined, but the Euler characteristic formula still applies.
In fact, transporting an $e\times m$ dyon along a non-orientable loop yields a negative phase change!
}

\subsection{Ground states}
Suppose $\ket{\Psi}$ is a ground state of the system, meaning $A_s \ket{\Psi} = B_p \ket{\Psi} = \ket{\Psi}$.
Let us work in the $\sigma^z$-basis, and label each classical spin configuration as $s: j \mapsto s_j \in \{\pm 1\}$.
The plaquette operator condition is met for classical configurations satisfying $B_p\ket{s} = w_p(s)\ket{s} = \ket{s}$,
where $w_p(s) := \prod_{j \in \bdry{p}} s_j$ is called the \emph{vortex} on plaquette $p$.
Then, the ground state is described as
\[
	\ket{\Psi} = \sum_{s : \forall p.\; w_p(s) = +1} c_s \ket{s},
\]
that is, a superposition of vortex-less configurations.

If a star operator $A_s$ acts on the classical state $\ket{s}$,
then the resulting state is also a classical configuration, with spins on $\star{s}$ flipped.
Therefore, $c_s$ and $c_{s'}$ must be equal if $s$ and $s'$ can be connected via star operators, i.e.,
if they lie in a common orbit of the star operators,
or in topological terms, a common \emph{cohomology class} of the $\mathbb{Z}_2$ 1-form gauge fields over $\Sigma$.

To decipher the last part, we define the terms in steps.
A \emph{G 1-form} at a point $p \in \Sigma$, for an abelian group $G$,
is a function $A$ that takes a tangent vector at $p$ and outputs a scalar in $G$.
A \emph{1-form field} assigns a 1-form to each point in $\Sigma$.
A \emph{0-form field} is a scalar field $C : \Sigma \rightarrow G$,
whose \emph{differential} is defined as $dC := \partial_\mu C dx^\mu$ (for a given coordinate system $x^\mu$).
A \emph{1-form gauge field} is the equivalence class of 1-forms such that $A \sim B \iff \exists C.\; B = A + dC$.
Thus, the cohomology $H^1(\Sigma, G)$ is the classification of $G$ 1-form gauge fields.
On a cellularized graph, 1-forms are defined on each edge,
and the differential is defined as $dC(\langle i, j\rangle) := C(j) - C(i).$\footnote{%
The group $\mathbb{Z}_2 = \{\pm 1\}$ is defined multiplicatively, meaning
$dC(\langle i, j\rangle) = C(j) \cdot C(i)^{-1} = C(i) C(j)$.
}

In case of the toric code on a genus-$g$ closed surface,
the cohomology group is given as $H^1(\Sigma_g, \mathbb{Z}_2) \cong (\mathbb{Z}_2)^{2g}$,
which has $4^g$ elements as expected.
This can be intuitively explained by the 2 incontractible loops per genus.
In fact the corresponding \emph{electric Wilson loop operators}
\[
	W_l^{(e)} := \prod_{j \in l} \sigma_j^z,
\]
defined the loop $l$ connects sites along the edges,
commute with every $A_s$, meaning these $2g$ operators allow us
to distinguish the degenerate states starting from any one of them.

The above derivation could have started with the $\sigma^x$ basis,
in which case the \emph{magnetic Wilson loop operators}
\[
	W_{l^*}^{(m)} := \prod_{j \in l^*} \sigma_j^x
\]
allow the complete labeling, where the loop $l^*$ connects plaquette centers penetrating the edges.
In fact, if $l \cap l^*$ contains a single site, then $W_l^{(e)}$ and $W_{l^*}^{(m)}$ anticommute,
acting as Pauli operators $X$ and $Z$ on the fourfold degenerate states traversed by them.

\subsection{Excitations}
Different energy eigenspaces can be traversed using non-loop path operators.
That is, for a curve $l$ with boundaries, the electric path operator $W_l^{(e)}$ (defined similarly)
commutes with all star operators except those at the boundaries, with which it anticommutes.
Thus, $W_l^{(e)}$ negates the eigenvalues of the star operators at its endpoints,
raising the energy in total by $4J_e$.
One says that a site with $A_s = -1$ has an \emph{electric charge} at that site,
and thus interpret $2J_e$ as the excitation energy of a single electric charge.
(An electric charge cannot be created in isolation, however; it is simply a mental description.)
Similarly, the magnetic path operator $W_{l^*}^{(m)}$ negates the eigenvalues of $B_p$ at its endpoints,
creating two vortices or \emph{magnetic charges}, each with excitation energy $2J_m$.

% =========================================
\section{Abelian anyons and quasiparticle statistics}
Recall from undergraduate quantum mechanics that, in three dimensions,
the indistinguishability of identical particles constrains the wavefunction
to be either symmetric or antisymmetric under exchange.
Let us use a simple game to motivate why that is and what changes in two dimensions.
Imagine that two well-separated quasiparticles are prepared in with definite positions,
and an adversary transports them along some paths to exchange their positions.
If the system remains gapped throughout the process and the transport is adiabatic,
then small deformations of the paths does not modify the net effect of this exchange.
Hence, the resulting transformation depends only on the homotopy class of the paths.
If the particles have long-range interactions, it would incur an additional,
path-dependent phase to the effect.

The configuration of two particles is labeled by
\[
    F_2(\mathbb{R}^3) := \left\{
        (\mathbb{x}, \mathbb{y}) \in \mathbb{R}^3 \times \mathbb{R}^3 |
        \mathbb{x} \neq \mathbb{y}
    \right\}.
\]
The indistinguishability of particles means the exchange
$(\mathbb{x}, \mathbb{y}) \mapsto (\mathbb{y}, \mathbb{x})$
correspond to the same state, meaning the unordered configuration space is
\[
    C_2(\mathbb{R}^3) := F_2(\mathbb{R}^3) / S_2.
\]
This space can be labeled with the center-of-mass coordinates
\[
    \mathbb{r}_{CM} = \frac{\mathbb{x} + \mathbb{y}}{2},\quad
    \mathbb{r}_{rel} = \mathbb{y} - \mathbb{x},
\]
with the exchange identification given by $\mathbb{r}_{rel} \sim -\mathbb{r}_{rel}$.
Thus, the configuration space is identified as
\[
    C_2(\mathbb{R}^3)
    \simeq \mathbb{R}^3 \times \frac{\mathbb{R}^3 \setminus \{0\}}{\sim}
    \simeq \mathbb{R}^3 \times \mathbb{RP}^2 \times \mathbb{R}_{>0}.
\]

For two particles in three dimensions, exchanging them twice is
topologically equivalent to doing nothing,
as the two closed paths can be continuously untangled and shrunk to zero.
(In fact, the fundamental group of the configuration space is given by
\[
    \pi_1(C_2(\mathbb{R}^3)) \simeq \mathbb{Z}_2,
\]
with $+1$ equivalent to no-op and $-1$ to a single exchange of the particles.)
Therefore, if one exchange acts on a one-dimensional state space
by a phase $e^{i\theta}$, performing it twice gives the identity and $e^{2i\theta}=1$.
The exchange phase is consequently restricted to $+1$ for bosons or $-1$ for fermions.
(Enter spin-statistics theorem... Wrong room? Okay...)\footnote{%
For particles with internal degrees of freedom like spin,
the state space may be multidimensional and the exchange would be represented
by a unitary operator $U$ such that $U^2 = 1$.
However, since the eigenvalues of $U$ would then be restricted to $\pm 1$,
the internal state space would be decomposed into the eigenspaces of $U$,
some of them symmetric and some antisymmetric under particle exchange.
Thus, internal degrees of freedom do not allow arbitrary exchange phases in three dimensions.
}

In two dimensions, exchanging two particles twice is instead homotopic to
winding one particle once around the other,
and this path cannot generally be untangled.
The winding number of the path is a complete homotopic invariant for two particles;
for many particles, the full braiding must be considered.
Thus, a two-dimensional quasiparticle can acquire an exchange phase
$e^{i\theta}$ for a general value of $\theta$.
Quasiparticles whose winding act as phases in this way are called \emph{Abelian anyons}.

The winding experiment also makes sense when the two particles are indistinguishable,
say $a$ and $b$.
An exchange would yield a nontrivial \emph{braiding operator} $R_{ab}$,
as a single exchange yields a measurably distinct state.\footnote{%
For non-abelian anyons, the state space of two particles is (apparently)
not the simple tensor product of the individual spaces.
It defines \emph{fusion spaces} and something something...
\hwnote{Help!}
}
The full winding $R_{ba} \cdot R_{ab}$, called their \emph{mutual statistics},
should then be a phase factor $e^{i\theta}$.
In the toric code, for example, the $e$ and $m$ particles
have a nontrivial mutual statistics of $-1$.

\subsection{Superselection sectors and fusion rules}
Two (composite) particles are defined to be \emph{of the same type} iff
a operator defined on a finite region can turn one into the other.
For example, two $e$ particles are equivalent to no particle,
as a string operator connecting the two can annihilate them.
States corresponding to the different particle types in this context
are called the \emph{superselection sectors}.\footnote{%
The name is a generalization of the \emph{selection sector},
where two states $\ket{\psi_1}$ and $\ket{\psi_2}$ are separated by a selection rule
if $\braket{\psi_1 | H | \psi_2} = 0$.
In this sense, they are separated by a superselection rule
if $\braket{\psi_1 | A | \psi_2} = 0$ for all operators with a bounded support.
}
With $1$ denoting the vacuum sector, one uses the notation $e\times e = 1$
to indicate that two $e$ particles are equivalent to vacuum;
such statements are called \emph{fusion rules}.

\subsection{Mutual statistics implies degeneracy on the torus}
Suppose two particles $e$ and $m$ with mutual statistics $-1$ exist on a genus-$g$ surface.
This fact alone, with no reference to the underlying model,
implies an at least $4^g$-fold degeneracy of the vacuum sector.
To prove this fact, let us label the $2g$ nontrivial loops on the surface as $1, \dots, 2g$.
Let $X$ and $Z$ each denote the operator that repsectively creates a pair of $e$ or $m$ particles,
transports one of the particles around loop $1$ or $2$, and annihilates the pair.
It can be shown that the operator $Z^{-1}X^{-1}ZX$
can be deformed into a single winding between $e$ and $m$,
preceeded by the pair creations and succeeded by annihilations.
\hwnote{Why?}
That is,
\[
    Z^{-1}X^{-1}ZX = -1 \Leftrightarrow \{X,Z\} = 0.
\]
Thus, these two non-commuting operators imply an at least 4-fold degeneracy of each sector.
Applying this argument the other $g-1$ loop pairs as well,
we find an at least $4^g$-fold degeneracy.
The pairs of loops should intersect at an odd number of points without tangents,
which is homotopically well-defined.

\subsection{The toric code in a field: perturbation analysis}
Applying a weak uniform magnetic field, one obtains the Hamiltonian:
\[
    H = -J_e \sum_s A_s - J_m \sum_p B_p - \sum_j (h_x \sigma_j^x + h_z \sigma_j^z).
\]
This destroys the exact solvability of the model, but it allows the particles to hop around the graph.
In fact, in the tight-binding limit (i.e., a single term in the Feshbach series),
the energy of a single particle (in the appropriate sense as before)
is given by the cosine dispersion:
\[
    \epsilon_e(q_x, q_y) \approx 2J_e - 2h_z (\cos q_x a + \cos q_y a)
\]
\[
    \epsilon_m(q_x, q_y) \approx 2J_m - 2h_x (\cos q_x a + \cos q_y a)
\]
(The $h_x$ field does not affect $\epsilon_e$ and vice versa,
as the $\sigma_j^x$ operator acting on a state with two $e$ particles
creates two $m$ particles, leaving the sector of our interest.)

In the case these fields are strong, the topological phase is destroyed,
and the system becomes polarized along the field.
This implies a phase transition between the topological phase and the paramagnetic phase.
Approaching this transition from the topological phase,
increasing $h_z$ gradually creates more and more $e$ particles,
which is refered to as \emph{Bose condensation} of the charges,
or of the vortices in the $h_x$ case.
However, simply rotating the field in the paramagnetic phase does not incur a phase transition.
Hence, the charge condensate and the vortex condensate phases are equivalent.

\subsection{Robustness of the topological degeneracy}
Recall that in the zero-field limit, the Wilson loop operators traverse between the degenerate ground states.
Thus, a similar analysis to that of section~\ref{sec:ising} lets us find
the ground state energy splitting to be
\[
    \Delta E \sim J e^{-L/\xi}\quad \text{with}\quad
    \xi^{-1} \sim \ln \frac{J}{h}
\]
where $L$ is the length of the loop, with appropriate subscripts for $J, h$.
In fact, to lowest order, the effective Hamiltonian is a linear combination of the
loop operators $X_i$ and $Z_i$,
as higher-order terms are exponentially suppressed.
However, it should be noted that the edge-localized operators in the Majorana chain
and the noncontractible Wilson loops in the toric code
are physically distinct mechanisms for creating topological degeneracy.
The relevant fact is that in either case, local perturbation cannot distinguish
the ground states at the lowest order.

\subsection{Emergent symmetry: gauge formulation}
On each vertex, one introduces an extra spin $\mu_v$ in the triplet state:
\[
    \frac{1}{\sqrt{2}}(\ket{\uparrow} + \ket{\downarrow}),
\]
that is, $\mu_v^x \ket{\Psi} = \ket{\Psi}$.
Our goal is to utilize this redundancy of representation
to describe why there are (approximate) symmetry operators with nonlocal support
that generate (exponentially suppressed) degeneracy.

We define the \emph{gauge transformations} as those generated by
\[
    G_v := \mu_v^x \prod_{\langle u, v\rangle} \sigma_{uv}^x.
\]
One can find that while $\mu_v^x$ and $\sigma_{uv}^x$ commute with $G_v$,
$\mu_v^z$ and $\sigma_{uv}^z$ anticommute.
We want to reformulate the Hamltonian using gauge-invariant terms.
To that end, we define the following operators:
\[
    \begin{cases}
        \Tilde\sigma_{uv}^x &= \sigma_{uv}^x \\
        \Tilde\sigma_{uv}^z &= \mu_u^z \sigma_{uv}^z \mu_v^z \\
        \Tilde\mu_v^x       &= \mu_v^x \prod_{\langle u, v\rangle} \sigma_{uv}^x \\
        \Tilde\mu_v^z       &= \mu_v^z
    \end{cases}
    \Leftrightarrow
    \begin{cases}
    ...
    \end{cases}
\]
Among the operators with tilde, $\Tilde\mu_v^z$ is negated by $G_v$,
while the other three are gauge invariant.
The $\Tilde\mu_v^x$, in particular, is defined such that
the $\Tilde\mu$ and $\Tilde\sigma$ operators define separate Pauli algebras.
Defining the gauge-invariant star operators $\Tilde A_s$ and plaquette operators $\Tilde B_p$,
the Hamiltonian is expressed as:
\[
    \Tilde H = -J_e\sum_v\Tilde A_v - J_m\sum_p\Tilde B_p
    -\sum_{\langle u, v\rangle}\left(
        h_x\Tilde\sigma_{uv}^x + h_z\Tilde\mu_u^z\Tilde\sigma_{uv}^z\mu_v^z
    \right),
\]
with the physical subspace characterized by the gauge condition $\Tilde\mu_v^x\Tilde A_v = +1$ at every vertex $v$.
In fact, on the physical subspace, one writes $A_v = \Tilde A_v = \Tilde\mu_v$.

\section{The honeycomb lattice model}
The honeycomb model has spin-aligning interactions along each bond,
with the direction of the bond determining the type of the Pauli operator.
(See equation~(5.1) of the paper.)
The plaquette operators $W_p$ (see equation~(5.2)) all commute with the Hamiltonian,
but there are twice as many plaquettes as there are vertices or spins.
Hence, even if we decompose the Hilbert space labeled by the vortex configurations
(i.e., into the simultaneous eigenspaces of $W_p$),
this only constrains half of all spins in each sector.

\subsection{A (redundant) representation of a spin by 4 Majorana operators}
In the honeycomb model where $\sigma^\alpha\; (\alpha = x. y, z)$ all appear as distinct interaction terms,
it is more convenient to write the Pauli operators using four Majoranas $c, b^\alpha$
instead of the two Majoranas as in the Jordan-Wigner transformation:
\[
    \Tilde\sigma^\alpha := ib^\alpha c.
\]
Comparing the two-dimensional spin space and the four-dimensional Fock space,
the physical subspace can be defined by the constraint
\[
    D := b^x b^y b^z c = 1.
\]
The soundness of this representation is given by the two observations:
\begin{enumerate}
    \item $[\Tilde\sigma^\alpha, D] = 0$,
        implying that the physical subspace is left invariant.
    \item $\Tilde\sigma^i\Tilde\sigma^j = b^i b^j = D \cdot i\varepsilon_{ijk} \Tilde\sigma^k$,
        meaning the algebraic relations hold on the physical subspace.
\end{enumerate}
Thus, we identify the actual spin operators $\sigma^\alpha$ with the restricted action of $\Tilde\sigma^\alpha$ on the subspace $D = 1$.

\subsection{Solving the Honeycomb Model using Majoranas}
Now suppose we rewrite every spin operator $\sigma_j^\alpha$ with four Majoranas each, i.e., $c_j, b_j^\alpha$.
The $2^N$-dimensional Hilbert space is enlarged to the $4^N$-dimensional Fock space,
with the physical subspace defined by the gauge conditions:
\[
    D_j := b_j^x b_j^y b_j^z c_j = +1.
\]
Equivalently, we define the projection operator onto the physical subspace:
\[
    \Pi := \prod_j \left(\frac{1+D_j}{2}\right).
\]
This allows us to rewrite the Hamiltonian as:
\[
    H = \frac{i}{4} \sum_{\langle j, k\rangle} A_{jk} c_j c_k\quad\text{with}\quad
    A_{jk} = 2J_{\alpha(j,k)} (ib_j^{\alpha(j,k)}b_k^{\alpha(j,k)}),
\]
where $\alpha(j,k)$ denotes the orientation of the edge $\langle j, k\rangle$.
Despite the fact that $A_{jk}$ is operator-valued instead of components of a real skew-symmetric matrix,
each operator $b_j^\alpha$ appears exactly once in the Hamiltonian.
This means that defining $u_{jk} := ib_j^{\alpha(j,k)}b_k^{\alpha(j,k)}$,
every $u_{jk}$ commute with each other as well as with the Hamiltonian.
Thus, every configuration of $u_{jk} = \pm 1$ corresponds to a distinct, pairwise orthogonal
subspace of the full Fock space.
On a given subspace, then, $A_{jk}$ is truly a constant and the Hamiltonian becomes a quadratic Majorana system,
just like the one we discussed before!

Moreover, a choice of the $u_{jk}$ configuration necessarily defines the vortex configuration:
\[
    W_p = \prod_{j \in \partial p} D_j \cdot \prod_{\langle j, k\rangle \in \partial p} u_{jk},
\]
with $\langle j, k\rangle$ ordered from the even sublattice to the odd sublattice
(with a bipartite convention previously defined).
Thus, the search for the ground state could be reduced to
minimizing the quadratic Majorana energy over all vortex and $u_{jk}$ configurations,
ignoring the fact that there are enormously many possible configurations.
Fortunately, Lieb's theorem tells us that the ground state has no vortices, i.e., $W_p = +1$.

\subsection{Fermionic spectrum in the honeycomb lattice model}
In the vortex-less subspace, we can systematically choose
\[
    u_{jk} = \begin{cases}
        +1 & (j \in \text{Even sublattice}) \\
        -1 & (j \in \text{Odd sublattice})
    \end{cases}
\]
and attempt to diagonalize the quadratic Hamiltonian.
Exploiting the translational symmetry of the system,
we would expect a momentum-dependent dispersion relation
\[
    \epsilon(\vec{q}) = \pm |f(\vec{q})|
\]
for some complex function $f(\vec{q})$ that depends on the parameters $J_\alpha$.
A careful derivation yields the phase diagram in figure~(5.3) of the paper.
A mathematical criterion for the B phase is that $f(\vec{q})$ has two zeros,
which approach each other and vanish at the phase boundary.

\subsection{Quasiparticle statistics in the gapped phase}
The particle statistics of the gapped phases can be discussed using perturbative analysis.
Specifically, consider the $A_z$ phase, where $J_z \gg J_x, J_y$.
The $\sigma_i^z \sigma_j^z$ terms incur a large energy cost when spins are not aligned in the $z$ bonds.
Thus, for low-energy states, one may reasonably consider the sector where the $z$-bond dimers
are all in spin-aligned states, labeled $\ket{\Uparrow}$ and $\ket{\Downarrow}$.
These dimers can be understood as lying on the edges of a rectangular lattice,
with plaquettes and vertices corresponding to hexagons on alternating rows.
The Feshbach expansion yields a nonconstant term at the fourth order, given by:
\[
    H_{eff} = \text{const.} - \frac{J_x J_y}{16J_z^3} \sum_p Q_p,
\]
where the square plaquette operator is given as
\[
    Q_p = \sigma_{p1}^y \sigma_{p2}^x \sigma_{p3}^y \sigma_{p4}^x.
\]
Appropriate rotations and adjustments yield the toric code system.
\hwnote{Do this calculation explicitly! (See also:~\cite{kitaev_anyons})}
Thus, excitations in this phase are precisely the $e$ and $m$ particles,
or more precisely, the superselection sectors follow the same fusion rules as the toric code.
Vortices in the even or odd rows correspond to the $e$ and $m$ particles, respectively,
The $e \times m$ sector corresponds to the fermion sector $ic_i c_j$,
though a single $e \times m$ particle and a single fermion state are not exactly the same.
